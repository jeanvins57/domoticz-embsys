...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    fd_green = open("/sys/class/gpio/gpio24/direction", O_WRONLY);
    if (fd_green == -1) {
    ...
    fd_green = open("/sys/class/gpio/gpio24/value", O_WRONLY);
    if (fd_green == -1) {
    ...
    while (1)
    {
        ...
        //Si l'affichage est en cours on fait clignoter la led verte
        ...
        //Si l'humidité est supéerieure à 40.6% on allume la led rouge
        if (comp_data.humidity > 40.6) {
    ...
    fd_green = open("/sys/class/gpio/unexport", O_WRONLY);
    if (fd_green == -1) {
